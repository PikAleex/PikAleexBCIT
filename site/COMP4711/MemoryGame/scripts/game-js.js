...
function set_grid() {
    reload_btn.removeEventListener('click', reset_grid);
    let fc_inner;
    let fc_front;
    let fc_back;
    ...
    for (let i = 0; i < (level * level); ++i) {
        ...
        if ((rand_gen(1, level) == 1) && nums.length != 0) {
            let tmp = rand_gen(0, nums.length - 1);
            tn = document.createTextNode(nums[tmp]);
            nums.splice(tmp, 1);
        } else {
    ...
    while (nums.length != 0) {
        if (panes[i].textContent == "X") {
            panes[i].childNodes[0].childNodes[1].textContent = nums[0];
            nums.splice(0, 1);
        }
        ++i;
    }
...
function flipping() {
    ...
    if (this.textContent == "X" || (order.length == 0 && this.textContent != "1") || (this.textContent != (order[order.length - 1] + 1)) && order.length != 0) {
        reload_btn.removeEventListener('click', reset_grid);
        let tmp = document.getElementsByClassName("flip-card-inner");
        downgrade();
        for (let i = 0; i < tmp.length; ++i)
            tmp[i].removeEventListener('click', flipping);
        setTimeout(reset_grid, 3000);
        --score;
...
function rotating() {
    ...
    reload_btn.addEventListener('click', reset_grid);
}
